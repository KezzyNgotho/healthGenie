    ...
    rows.push(
      <View key={i} style={styles.levelRow}>
        {rowLevels.map((level, index) => (
          <TouchableOpacity key={index} onPress={() => onSelect(level, category)} style={styles.radioButton}>
...
const MentalHealthScreen = () => {
  ...
  const addMessage = () => {
    try {
      const { mood, anxietyLevel, sleepQuality, energyLevel, stressLevel } = assessmentDetails;
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <LevelSelector
        ...
        selectedLevel={mood}
        onSelect={(level) => handleLevelSelect(level, 'setm)}
      />
