...
const HealthHabitScreen = () => {
  ...
  const generateGoalPlan = async () => {
    try {
      const selectedHabitNames = selectedHabits.map((id) => habitList.find((h) => h.id === id).name);const prompt = `The user wants to create a health goal plan. They have selected the following health habits:\n- ${selectedHabitNames.join('\n- ')}.\nThe user's goal description is: ${goalDescription}`;
