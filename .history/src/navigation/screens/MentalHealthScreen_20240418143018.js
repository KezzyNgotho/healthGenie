...
const MentalHealthScreen = () => {
  ...
  const addMessage = () => {
    if (message.trim() !== '') {
      ...
      processUserMessage(message); // Call the API to get AI's response
      setMessage('');
    }
  };
...
Voice.onSpeechResults = (e) => {
  ...
  const sendToGeminiAI = async () => {
    const apiKey = 'bab1ac0b9e88c148fd7e07863c4447d7b28b4ab7';
    const apiUrl = 'https://us-east4-aiplatform.googleapis.com'; // Replace with your API endpoint
    ...
    try {
      ...
      // Handle the response data (provide feedback or assistance to the user)
      console.log(responseData);
      // Here, you can display the response from Gemini AI to the user
      // For example:
      Alert.alert('Assistance', responseData.message || 'Assistance is on the way!');
    } catch (error) {
      console.error('Error sending data to Gemini AI:', error);
