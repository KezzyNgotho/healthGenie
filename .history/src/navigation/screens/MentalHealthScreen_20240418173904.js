...
Voice.onSpeechResults = (e) => {
  ...
  const processUserMessage = async (message) => {
    try {
      ...
      // Check if AI response is iterable
      if (aiResponse && typeof aiResponse[Symbol.iterator] === 'function') {
        // Iterate over the AI response
        for (const item of aiResponse) {
          // Do something with each item (if needed)
        }
      } else {
        console.error('AI response is not iterable');
      }
